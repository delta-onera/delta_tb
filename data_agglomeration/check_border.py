...
earlystopping = miniworld.getrandomtiles(50, 128, 32)
...
for epoch in range(nbepoch):
    ...
    XY = miniworld.getrandomtiles(100, 128, batchsize)
    for x, y in XY:
        ...
        if True:  # False and random.randint(0, 3) != 0:
            with torch.no_grad():
                ...
                torchvision.utils.save_image(
                    torch.cat([rahhh, rahhh, rahhh], dim=1) * 0.5, "build/original.png"
                )
                rahhh = yy.unsqueeze(1)
                torchvision.utils.save_image(
                    torch.cat([rahhh, rahhh, rahhh], dim=1) * 0.5,
                    "build/borderless.png",
                )
