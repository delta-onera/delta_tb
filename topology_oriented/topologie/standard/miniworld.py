...
if __name__ == "__main__":
    a = (torch.rand(8, 8) * 10).int()
    print("#########")
    print(a)
    for i in range(11):
        print(i, (a == i).float().sum())
    print("#########")
    print(sortmap(a.numpy()))
    print("#########")

    root = "build/"

    y = PIL.Image.open(root + "16_y.png").convert("L").copy()
    y = numpy.uint8(numpy.asarray(y))
    y = numpy.uint8(y != 0)

    z = PIL.Image.open(root + "16_z.png").convert("L").copy()
